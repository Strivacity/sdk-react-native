#import "StrivacityReactNative.h"
...
@interface StrivacityReactNative()<StrivacityReactNativeAuthFlowManagerDelegate> {
    id<OIDExternalUserAgentSession> _currentSession;
}
@end

@implementation StrivacityReactNative
...
UIBackgroundTaskIdentifier strivacityReactNativeTaskId;
                            ...
                            reject: (RCTPromiseRejectBlock)  reject
{
    ...
    id<UIApplicationDelegate, StrivacityReactNativeAuthFlowManager> appDelegate = (id<UIApplicationDelegate, StrivacityReactNativeAuthFlowManager>)[UIApplication sharedApplication].delegate;
    if (![[appDelegate class] conformsToProtocol:@protocol(StrivacityReactNativeAuthFlowManager)]) {
        [NSException raise:@"StrivacityReactNative Missing protocol conformance"
                    format:@"%@ does not conform to StrivacityReactNativeAuthFlowManager", appDelegate];
    }
    appDelegate.authorizationFlowManagerDelegate = self;
    __weak typeof(self) weakSelf = self;

    strivacityReactNativeTaskId = [UIApplication.sharedApplication beginBackgroundTaskWithExpirationHandler:^{
        [UIApplication.sharedApplication endBackgroundTask:strivacityReactNativeTaskId];
        strivacityReactNativeTaskId = UIBackgroundTaskInvalid;
    }];
